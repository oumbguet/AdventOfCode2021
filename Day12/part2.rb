...
class Graph
    ...
    def find_paths(current, twice)
        ...
        current.neighbours.each do |n|
            if n.visited && !n.big
                if twice && n.value != 'start'
                    local_count += find_paths(n, false)
                else
                    next
                end
            else
                local_count += find_paths(n, twice)
                n.visited = false
            end
        end
...
puts graph.find_paths(graph.find_node_by_value('start'), twice=true)